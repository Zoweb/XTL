...
const Tag = require("../api/javascript/tag");
const TagMeta = require("../api/javascript/tag-meta");
...
function processTag(jsActions, previousTag, tag) {
    if (tag.data.flags.compiled && jsActions[tag.name]) {
        ...
        actionMethod(new Tag(tag.name, Tag.convertAttributesKeyVal(tag.attributes)))
